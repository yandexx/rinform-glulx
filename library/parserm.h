...
[ KeyboardPrimitive  a_buffer a_table done ix;
    ...
    glk($0141, gg_mainwin, a_buffer+WORDSIZE, INPUT_BUFFER_LEN-WORDSIZE, 0); ! request_line_event
    while (~~done) {
        glk($00C0, gg_event); ! select
        switch (gg_event-->0) {
          5: ! evtype_Arrange
...
[ ParseToken given_ttype given_tdata token_n x y;
    ...
    if (y == GPR_REPARSE) VM_Tokenise(buffer,parse);
